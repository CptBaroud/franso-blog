...
I hope this analogy has helped you understand the client/server principle.

## State-what ? Stateless ?

You may have heard the term STATELESS before, but you don't necessarily understand what it means for HTTP => The fact that HTTP is "stateless" (as opposed to statefull) means that each HTTP request is independent of the other, and that the server doesn't remember the state or history of previous requests.
...
* A URL (Uniform Resource Locator) is a Web address that identifies the location of a specific resource on the Internet, such as a Web page, an image or a downloadable file. 
    ...
    For example, https://www.google.com/search?q=chat is a URL that points to the search results page for the word "chat" on Google.
...
In short, all URLs are URIs, but the converse is not true.
Still, they have the same purpose, to serve as identifiers for locating resources on the Web, with differences in their specification and operation.
...
We won't go into the details of the TCP protocol in this psot, but simply put, TCP defines how data is formatted, then sent from point A to point B.
...
These are the main HTTP verbs, others exist, but we'll quickly go over them because they don't eat bread:
...
The history of HTTP versions could be the subject of a whole video, but just remember that in HTTP version 0/9, each request opened a TCP/IP connection, which wasn't very opti, and only the verb is the resource was present.

HTTP version 1/0 introduced headers, which we'll see later.
...
Finally, version 2 introduced binary rather than text encoding, and protocol multiplexing to manage multiple requests within a single TCP/IP connection (the keep-alive header made this possible in 1/1, but that's it).
...
For example, a Content-Type header indicates the type of content sent or received, such as text, image or video. 
...
* HTTP response codes in the 1xx series are information responses and are used to indicate that the server has received the client's request and is continuing to process it. For example, 101 (Switching Protocols): indicates that the server has accepted the protocol switching proposal sent by the client.

* HTTP response codes starting with 2xx indicate that the request has been processed successfully. For example, HTTP response code 200 means that the request has been processed successfully and that the response contains the requested information.

* HTTP response codes beginning with 3xx indicate a redirection. For example, HTTP response code 301 indicates that the requested resource has been permanently moved to a new address, and that the client needs to update its URL.

* HTTP response codes starting with 4xx indicate an error caused by the client. For example, HTTP response code 404 means that the requested resource was not found on the server.

* HTTP response codes starting with 5xx indicate an error caused by the server. For example, HTTP response code 500 means that an error occurred on the server side when processing the request.